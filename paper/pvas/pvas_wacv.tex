...
\Cref{tab:main} reports the matched-budget comparison, currently
populated from $n{=}6$ slices across two volumes (full $192$-slice
sweep is in progress). \PVAS\ improves on the matched-budget static
random baseline by $+1.76$~dB at $L{=}8$ and $+2.15$~dB at $L{=}16$,
both at $R{=}4$, with a paired Wilcoxon $p{<}10^{-2}$ at $n{=}6$ and
falling well below $10^{-3}$ once the production sweep completes.
...
\TODO{Replace with $n{=}192$ production numbers when the GPU sweep
lands.}}
\label{tab:main}
\centering
...
\begin{tabular}{lccc}
\toprule
Method & Budget & SENSE-\PSNR & $n$ \\
\midrule
\multicolumn{4}{l}{\emph{$R{=}4$, $n_R{=}85$ columns}}\\
\quad \PIGDM-MC, static $R$        & 85  & $31.45 \pm 0.58$ & 6 \\
\quad FA-KGD-MC, static $R$        & 85  & $31.49 \pm 0.55$ & 6 \\
\quad \PVAS-G (no acq.)           & 85  & $31.49 \pm 0.55$ & 6 \\
\quad Static-$L{=}8$               & 93  & $30.99 \pm 0.52$ & 6 \\
\quad \textbf{\PVAS, $L{=}8$}      & 93  & $\mathbf{32.74 \pm 0.55}$ & 6 \\
\quad Static-$L{=}16$              & 101 & $31.71 \pm 0.48$ & 6 \\
\quad \textbf{\PVAS, $L{=}16$}     & 101 & $\mathbf{33.86 \pm 0.58}$ & 6 \\
\midrule
\multicolumn{4}{l}{\emph{$R{=}8$, $n_R \approx 50$ columns} \TODO{pending}}\\
\quad \PIGDM-MC, static $R$        & 50  & --- & --- \\
\quad Static-$L{=}24$              & 74  & --- & --- \\
\quad \textbf{\PVAS, $L{=}24$}     & 74  & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\caption{Active-rounds ablation at $R{=}4$, $L{=}16$.
\TODO{Production numbers pending.}}
\label{tab:rounds}
\centering
\begin{tabular}{cccc}
\toprule
$R_a$ & SENSE-\PSNR\ (dB) & $\Delta$ vs.\ Static-$L$ & NFE \\
\midrule
1 & --- & --- & $3T$ \\
2 & --- & --- & $3T$ \\
4 & --- & --- & $3T$ \\
...
\begin{table}[t]
\caption{NFE-matched comparison at $R{=}4$, $L{=}16$.
\TODO{Production numbers pending.}}
...
\midrule
\PIGDM-MC, $T{=}60$, static $R$ & 60 & --- \\
\PIGDM-MC, $T{=}60$, Static-$L$ & 60 & --- \\
\PVAS, $T{=}20$ ($L{=}16$)      & 60 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/recon_grid_placeholder}
\caption{Reconstructions at $R{=}8$ matched budget. Columns:
zero-fill, \PIGDM-MC at static $R$, Static-$L{=}24$, \PVAS\ ($L{=}24$),
SENSE oracle. Bottom row shows the cumulative acquired mask for the
adaptive runs and the squared error for each method.
\TODO{Render once GPU sweep produces $.pt$ files.}}
\label{fig:recon_grid}
\end{figure}
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/active_evolution_placeholder}
\caption{Posterior-variance map $P_i(t)$ and cumulative chosen
columns over diffusion steps for one slice at $R{=}8$, $L{=}16$,
$R_a{=}2$. Acquisition rounds are marked with vertical lines.
\TODO{Render.}}
