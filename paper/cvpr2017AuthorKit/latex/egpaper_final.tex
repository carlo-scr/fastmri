...
$+0.39$~dB and over DPS by $+2.0$ to $+3.4$~dB. ADPS underperforms DPS at these step budgets because its
...
brain $R{=}4$, $T{=}50$ slice. Columns: GT, $\Pi$GDM, FA-KGD, signed
...
crops of \Cref{fig:recon} (brain $R{=}4$, $T{=}50$). Crop $\Delta$ exceeds
the slice $\Delta$: the gain concentrates at edges.}
\label{tab:crops}
...
\begin{table}[H]
\centering
\caption{Brain step-budget sweep ($192$ slices, $12$ volumes, $\beta{=}5$
radial noise, analytic radial init). $\Delta$ is per-slice mean$\pm$std
with one-sided paired Wilcoxon $p$ ($n{=}192$); all $12$ volume means
...
brain-trained EDM, same hyperparameters as \Cref{tab:tscaling}). $\Delta$:
per-slice mean$\pm$std, one-sided paired Wilcoxon $p$ ($n{=}192$).}
...
\begin{table}[H]
\centering
\caption{Brain (matched) vs.\ knee (cross-domain) gains at $T{=}20$
(from \Cref{tab:tscaling,tab:tscaling_knee}); matched brain gains are
$\sim$$1.6{\times}$ cross-domain knee gains.}
...
\begin{table}[H]
\centering
\caption{Step-budget scaling, brain $R{=}4$ ($192$ slices, $12$ volumes,
oracle init). $\Pi$GDM peaks at $T{=}50$ then degrades; FA-KGD is monotone.}
\label{tab:scaling}
\setlength{\tabcolsep}{2.8pt}
\scriptsize
...
$\Delta$PSNR (dB) over $\Pi$GDM across $\beta$ and $\alpha$. All $12$
settings improve; best in \textbf{bold}.}
\label{tab:hparam}
\setlength{\tabcolsep}{6pt}
\small
...
sweeps (\Cref{tab:tscaling,tab:tscaling_knee}) all use $192$ slices from
