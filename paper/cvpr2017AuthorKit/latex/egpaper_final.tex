...
(i)~Pilot main table (\Cref{tab:results}): five consecutive central
slices from one validation volume per anatomy/setting (treated as directional
evidence).
...
the sampler does not converge and the pilot numbers reflect the same
...
per setting). Best training-free result in \textbf{bold};
E2E-VarNet~\cite{sriram2020varnet} (grey) is a supervised, brain-only
reference.}
\label{tab:results}
\small
...
\begin{table}[H]
\centering
\caption{Cross-domain summary across the four pilot settings of
\Cref{tab:results}. Brain (matched) gains are $2$--$3{\times}$ knee
(cross-domain), mirroring \Cref{tab:tscaling,tab:tscaling_knee}.}
...
\begin{table}[H]
\centering
\caption{Per-band k-space NMSE on brain $R{=}4$ (5 slices). Relative
improvement is $(\Pi\text{GDM}{-}\text{FA-KGD})/\Pi\text{GDM}$; gains
concentrate at low frequencies.}
\label{tab:freq}
\setlength{\tabcolsep}{5pt}
\small
...
\midrule
0--50    & 0.0182 & 0.0166 & $\mathbf{+9.1}$ \\
50--100  & 0.0291 & 0.0266 & $\mathbf{+8.7}$ \\
100--150 & 0.0418 & 0.0409 & $+2.1$ \\
150--200 & 0.0556 & 0.0555 & $+0.2$ \\
200+     & 0.0721 & 0.0720 & $+0.1$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Hyperparameter sweep on brain $R{=}4$, $T{=}20$ (5 slices):
$\Delta$PSNR (dB) over $\Pi$GDM across FPDC exponent $\beta$ and EMA
smoothing $\alpha$. All $12$ settings improve; best in \textbf{bold}.}
\label{tab:hparam}
\setlength{\tabcolsep}{6pt}
\small
...
\cmidrule{2-5}
 & 0.80 & 0.90 & 0.95 & 1.00 & \\
\midrule
0.5 & $+0.311$ & $+0.311$ & $+0.311$ & $+0.310$ & $+0.311$ \\
1.0 & $+0.312$ & $+0.313$ & $\mathbf{+0.315}$ & $+0.313$ & $+0.313$ \\
2.0 & $+0.229$ & $+0.228$ & $+0.229$ & $+0.232$ & $+0.230$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Limitations.}
The main brain/knee table remains a five-slice pilot; the $192$-slice
brain and knee sweeps (\Cref{tab:tscaling,tab:tscaling_knee}) carry the
step-budget claim with volume-level significance, but both use a
...
$\beta$ and EMA smoothing $\alpha$ on brain $R{=}4$, expanding
\Cref{tab:hparam} of the main paper. All twelve combinations improve over
$\Pi$GDM. The plateau across $\beta\!\in\!\{0.5,1.0\}$ and
$\alpha\!\in\!\{0.8,0.95\}$ shows that FA-KGD is not narrowly tuned: any
moderate schedule that defers high-frequency DC is within $0.01$~dB of the
optimum $(\beta{=}1.0,\alpha{=}0.95,\Delta{=}{+}0.315$~dB$)$. The drop at
$\beta{=}2.0$ reflects an overly aggressive late-stage radius expansion
that re-introduces the DC-shock failure mode FPDC was designed to remove.
...
($T{=}100$). Final-iterate PSNR on the same five-slice development set
