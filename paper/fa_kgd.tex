...
\textbf{FA-KGD}, a training-free sampler that corrects both errors with a
unified update. A frequency-dependent noise covariance $\hatR$ is estimated
directly from the Auto-Calibration Signal (ACS) lines of the scan itself and
yields a closed-form, timestep-adaptive Kalman gain $K_i(t)$. Data
consistency is then restricted to a frequency radius $r(t)$ that expands as
...
EDM~\cite{karras2022edm} checkpoint, FA-KGD improves $\Pi$GDM by $+0.12$ to
$+0.49$~dB PSNR and up to $+0.012$ SSIM; gains grow with the step budget
...
method adds $\sim$150 lines to an open-source sampler, introduces no new
weights, and adds $<\!1\%$ per-step overhead.
\end{abstract}
...
is clinically bottlenecked by long acquisition times. \emph{Accelerated MRI}
...
as E2E-VarNet~\cite{sriram2020varnet} still hold the fastMRI leaderboard,
but require retraining for every anatomy, coil array, and acceleration
factor. Training-free diffusion samplers ---
DDRM~\cite{kawar2022ddrm}, DPS~\cite{chung2023dps},
...
\nabla_{x_t} \log p(x_t)$ to reverse it, generalised to continuous-time SDEs
by Song et al.~\cite{song2021score} and to the practically dominant EDM
parameterisation by Karras et al.~\cite{karras2022edm}. At inference, each
step produces a Tweedie-denoised estimate
$\muth(x_t) \!\triangleq\!\mathbb{E}[x_0|x_t]$ that we treat as the ``prior''
side of a measurement-prior fusion.

\subsection{Diffusion Samplers for Inverse Problems}

DDRM~\cite{kawar2022ddrm} uses the SVD of the forward operator and solves
noisy linear inverse problems in ${\sim}20$ steps. DPS~\cite{chung2023dps}
back-propagates a Laplace-approximated likelihood gradient through the
Tweedie estimate. $\Pi$GDM~\cite{song2023pigdm}, our primary baseline,
derives the exact posterior update for the linear-Gaussian model:
\begin{equation}
...
every measured k-space location. ADPS~\cite{daras2025ambient} extends the
family to ambient-trained models via annealed Langevin likelihood guidance;
its default schedule targets $T\!\geq\!300$ and underperforms at the low step
budgets we evaluate.
...
jointly estimates image and degradation, but its M-step is a per-step MAP
optimisation; ours is a single element-wise update. Neither work addresses
...
$N_c\!\approx\!16$ coils. The $\sim$10~dB gap in Table~\ref{tab:results}
reflects this information-access difference (multicoil raw data vs.\ a single
RSS-combined channel) as much as model capacity; it is orthogonal to the
...
Table~\ref{tab:results} reports PSNR and SSIM across brain and knee at two
...
(§\ref{sec:discussion}). The supervised E2E-VarNet reaches 39.91~dB on
brain $R{=}4$ --- roughly 10~dB above any training-free result, a gap that
we have already attributed (§\ref{sec:related}) to its multicoil raw-data
...
($+0.30$~dB at $T{=}20$ to $+0.50$~dB at $T{=}50$, Fig.~\ref{fig:scaling}a);
...
\begin{figure}[t]
  ...
  in Table~\ref{tab:results}. (a)~$\Delta$PSNR and (b)~$\Delta$SSIM relative
...
$T$ (Fig.~\ref{fig:scaling}), so it should reach a target quality at fewer
steps than $\Pi$GDM --- a practical hypothesis worth testing at scale.
...
\begin{itemize}
  ...
  brain-trained EDM cross-domain; a knee-matched checkpoint would likely
  close the brain--knee gain gap.
  \item \textit{RSS magnitude only.} Current experiments use a single
  RSS-combined channel. Extending FA-KGD to a multi-coil SENSE formulation
  with phase information is the most direct path to narrowing the
  supervised gap (§\ref{sec:related}) while retaining training-free
  operation.
\end{itemize}
