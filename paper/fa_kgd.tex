...
volume. The per-frequency noise process is therefore observed $N_z$ times
over, with statistics that are stationary across $z$ to a very good
approximation. We exploit this by pooling the ACS estimator volumetrically:
\begin{equation}
  \hatsig_i^{2,\text{3D}} \;=\; \frac{1}{N_z(N_c-1)}
  \sum_{z=1}^{N_z}\sum_{c=1}^{N_c}
  \bigl|y_{i,c,z} - \bar y_{i,z}\bigr|^2,
  \label{eq:acs3d}
\end{equation}
which raises the effective degrees of freedom from $N_c{-}1$ to
$N_z(N_c{-}1)$ and reduces the relative MSE of $\hatsig_i^2$ by a factor of
$N_z$. The change is purely statistical: \Cref{eq:acs3d} is computed once
per volume in a single pre-pass, then reused as the $\hatsig_i^2(0)$
initialisation of \Cref{alg:fakgd} for every slice. No architectural,
training, or per-step changes are required, and the training-free property
is preserved.

\textbf{Single-replicate degeneracy.} For pipelines that operate on root-sum-of-squares (RSS) coil-combined slices --- as ours does for parity
with the EDM checkpoint --- the per-slice estimator collapses: with one
replicate per pixel, the sample variance is undefined. Cross-slice pooling
is then the \emph{minimal} way to recover any ACS-based noise estimate at
all. Pooling therefore both stabilises the multi-coil estimator and
\emph{enables} self-calibration in single-coil settings where it would
...
estimator has 26\% relative RMSE on $\hatsig_i^2$; the pooled estimator
drops this to 6.3\%, an empirical $17\times$ MSE reduction matching the
theoretical $N_z\!=\!16$ prediction. After the radial polynomial fit, the
recovered full-grid $\hatsig_i^2$ map has $0.8\%$ mean relative error
...
\subsection{Volumetric vs Per-Slice ACS Estimation}
\label{sec:exp_acs3d}

We quantify the impact of the volumetric ACS pooling of
\Cref{sec:fakgd3d} on real fastMRI brain reconstruction. For each of
$N_v\!=\!10$ validation volumes we add the $\sigma^2(r)$ measurement noise
of \Cref{sec:experiments} with a per-volume seed, run the EDM checkpoint at
$T{=}20$, and aggregate metrics first within a volume (mean over slices)
and then across volumes (mean $\pm$ std). Three $\hatsig_i^2(0)$
initialisations are compared: \textsc{Oracle} (true $\sigma^2(r)$, upper
bound), \textsc{Pooled-ACS} (\Cref{eq:acs3d}, proposed), and $\Pi$GDM
(single isotropic $\sigma_y$, external baseline). In this RSS-combined
pipeline the per-slice ACS estimator is degenerate (\Cref{sec:fakgd3d}) and
is therefore omitted; pooled ACS is the natural entry point.

\begin{table}[t]
\centering
\caption{Volumetric ACS pooling on fastMRI multicoil-val (brain),
10 volumes, EDM, $T{=}20$. Per-volume mean $\pm$ std (full sweep pending;
placeholders marked TBD).}
\label{tab:acs3d}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lcccc}
\toprule
 & \multicolumn{2}{c}{$R{=}4$} & \multicolumn{2}{c}{$R{=}8$} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Method & PSNR & SSIM & PSNR & SSIM \\
\midrule
$\Pi$GDM (iso.\ $\sigma_y$)         & TBD & TBD & TBD & TBD \\
FA-KGD, \textsc{Pooled-ACS}         & TBD & TBD & TBD & TBD \\
FA-KGD, \textsc{Oracle} (UB)        & TBD & TBD & TBD & TBD \\
\bottomrule
\end{tabular}
\end{table}

The gain is expected to scale with $N_z$: brain volumes here have
$N_z\!\approx\!16$, the knee subset $N_z\!\approx\!36$, so the same
estimator yields a tighter $\hatsig_i^2$ map on knee data without any
change. Reproducibility: \texttt{scripts/run\_acs\_pooling\_sweep.sh}
produces the table from a clean checkout.
