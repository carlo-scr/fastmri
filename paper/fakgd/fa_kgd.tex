...
The current main table is a pilot evaluation: five consecutive slices from one
validation volume per anatomy/setting, with knee at $320\times320$ and brain
(axial T2-weighted, AXT2) center-cropped to $384\times320$. The larger
step-budget experiment in \Cref{tab:tscaling} uses $192$ AXFLAIR brain slices
from $12$ validation volumes. We report these sample sizes explicitly because
...
acceleration rates. Because this table currently uses only five slices per
setting, we treat it as directional evidence and rely on \Cref{tab:tscaling}
for the stronger sample-size claim. Among training-free methods FA-KGD wins every cell,
...
\begin{table*}[t]
\centering
\caption{Pilot quantitative comparison on fastMRI (five consecutive slices
from one validation volume per setting). Best training-free result per setting
in \textbf{bold}. E2E-VarNet~\cite{sriram2020varnet} (grey) is a supervised
reference that is only available for the brain multicoil setting.}
\label{tab:results}
\small
...
\cmidrule{3-7} \cmidrule{9-13}
 ...
 & DPS & ADPS & $\Pi$GDM & \textbf{FA-KGD} & VarNet$^\dagger$ \\
\midrule
Brain $R{=}4$ & 20 & 27.31 & 25.25 & 29.43 & \textbf{29.73} & \textcolor{gray}{34.60} & & .663 & .572 & .867 & \textbf{.871} & \textcolor{gray}{.856} \\
Brain $R{=}4$ & 50 & 28.07 & 26.44 & 30.00 & \textbf{30.49} & \textcolor{gray}{34.60} & & .721 & .642 & .864 & \textbf{.874} & \textcolor{gray}{.856} \\
...
but consistent ($+0.12$--$+0.14$~dB in the five-slice pilot,
...
\textbf{Limitations.}
\begin{itemize}
  \item \textit{Evaluation scale.} The main brain/knee table is still a pilot
  five-slice evaluation. The $192$-slice brain sweep ($12$ volumes,
