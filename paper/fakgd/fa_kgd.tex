...
The brain rows of \Cref{tab:results} and \Cref{tab:tscaling} aggregate $192$
AXFLAIR slices from $12$ validation volumes ($384\times320$); the knee rows
of \Cref{tab:results} and \Cref{tab:tscaling_knee} aggregate $192$
single-coil slices from $6$ validation volumes ($320\times320$, axial
T2-weighted, brain-trained EDM applied cross-domain). We report sample sizes
explicitly because slice-level statistics over adjacent slices are correlated
and should not be read as independent patient-level evidence.
...
single-coil, brain-trained EDM applied cross-domain; full knee step-budget
sweep in \Cref{tab:tscaling_knee}). Among training-free methods FA-KGD wins
every cell, improving over $\Pi$GDM by $+0.10$ to $+0.39$~dB and over DPS by
$+2.0$ to $+3.4$~dB. ADPS underperforms DPS at
...
AXFLAIR slices from $12$ validation volumes; knee rows aggregate $192$
single-coil slices from $6$ validation volumes (brain-trained EDM applied
cross-domain; full knee step-budget sweep in \Cref{tab:tscaling_knee}).
...
($R{=}4$: $+0.16\!\to\!+0.24\!\to\!+0.31\!\to\!+0.40$~dB; $R{=}8$:
...
\begin{figure}[H]
  ...
  EDM; illustrative pilot subset, see \Cref{tab:tscaling,tab:tscaling_knee}
  for the full $192$-slice numbers). (a)~$\Delta$PSNR (FA-KGD $-$ $\Pi$GDM)
  grows monotonically with $T$. (b)~Absolute PSNR: $\Pi$GDM peaks at $T{=}50$
  and slightly degrades afterwards, while FA-KGD remains flat, consistent
  with our hypothesis that FPDC eliminates the over-correction mode that
  drives $\Pi$GDM's late-$T$ regression. (c)~SSIM follows the same trend.
  \Cref{tab:tscaling} confirms the same pattern at scale on brain ($192$
  slices, $12$ volumes; peak $\Delta{=}+0.51$~dB at $T{=}100$) and
  \Cref{tab:tscaling_knee} replicates it cross-domain on knee ($192$
  slices, $6$ volumes), both for $R{=}4$ and $R{=}8$.}
  \label{fig:scaling}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Per-band k-space NMSE on brain $R{=}4$, $T{=}50$, computed from
  real reconstructions on $192$ AXFLAIR slices from $12$ volumes.
  (a)~Absolute NMSE by radial frequency band. (b)~Relative improvement over
  $\Pi$GDM per band. FA-KGD gains are concentrated in the low-frequency
  bands ($+14.0$\% at 0--50, $+8.1$\% at 50--100) and are near-zero at the
  $150$--$200$ band, consistent with the FPDC mechanism that defers
  high-frequency DC to later denoising steps.}
  \label{fig:freq}
\end{figure}
...
but consistent (matched setting in \Cref{tab:results}: $+0.24$~dB at $R{=}4$
and $+0.14$~dB at $R{=}8$, $T{=}20$,
...
\textbf{Limitations.}
\begin{itemize}
  \item \textit{Evaluation scale.} The brain and knee rows of
  \Cref{tab:results} and the step-budget sweeps
  (\Cref{tab:tscaling,tab:tscaling_knee}) aggregate $192$ slices from $\geq 6$
  volumes. The $192$-slice brain sweep ($12$ volumes,
