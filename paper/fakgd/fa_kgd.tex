...
EDM, cross-domain; full sweep in \Cref{tab:tscaling_knee}). Best
...
\begin{figure*}[t]
  ...
  domain-matched EDM). FA-KGD reduces residual energy at cortical and
  ventricular edges where $\Pi$GDM produces Gibbs-like ringing. Error maps
  share a colour scale; per-slice PSNR/SSIM overlaid.}
  \label{fig:recon}
\end{figure*}
...
\begin{table}[H]
\centering
\caption{Step-budget scaling, $192$ AXFLAIR brain slices ($12$ volumes,
$\beta{=}5$ radial noise, analytic radial init,
$m_{\text{step}}{=}\textsc{clamp}$, $\sigma_{\max}{=}10$).
$\Delta$: per-slice mean (FA-KGD$-\Pi$GDM)$\pm$std with one-sided paired
Wilcoxon $p$. All $12$ volume means positive
(volume Wilcoxon $p{=}2.44{\times}10^{-4}$).}
\label{tab:tscaling}
...
\Cref{tab:tscaling}). $\Delta$: per-slice mean (FA-KGD$-\Pi$GDM)$\pm$std,
one-sided paired Wilcoxon $p$ ($n{=}192$).}
...
\begin{figure}[H]
  ...
  EDM; pilot subset). (a)~$\Delta$PSNR (FA-KGD $-$ $\Pi$GDM) grows
  monotonically with $T$. (b)~$\Pi$GDM peaks at $T{=}50$ then degrades;
  FA-KGD stays flat, consistent with FPDC eliminating $\Pi$GDM's late-$T$
  over-correction. (c)~SSIM follows the same trend. Full $192$-slice numbers
  in \Cref{tab:tscaling,tab:tscaling_knee}.}
  \label{fig:scaling}
\end{figure}
...
\begin{figure}[H]
  ...
  $12$ volumes). (a)~Absolute NMSE by radial band. (b)~Relative improvement
  over $\Pi$GDM. FA-KGD gains concentrate at low frequencies, consistent
  with FPDC deferring high-frequency DC.}
  \label{fig:freq}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Cross-domain comparison across the four \Cref{tab:results}
  settings: (a)~$\Delta$PSNR and (b)~$\Delta$SSIM over $\Pi$GDM. FA-KGD
  wins every cell; matched brain (blue) gains are $2$--$3{\times}$ the
  cross-domain knee (orange) gains in PSNR, comparable in SSIM.}
...
\begin{figure}[t]
  ...
  $12$ volumes): $\Delta$PSNR over $\Pi$GDM across FPDC exponent $\beta$ and
  EMA $\alpha$ ($12$ configs). Optimum $\beta{=}1.0$, $\alpha{=}0.95$
  ($\Delta{=}{+}0.232$~dB); all $12$ improve. Flat across $\alpha$ and within
  $0.001$~dB across $\beta\!\in\!\{0.5,1.0\}$, slightly worse at
  $\beta{=}2.0$: the existence of a progressive schedule matters more than
  its exact shape.}
  \label{fig:hparam}
\end{figure}
