...
\begin{frame}{Method overview}
  \begin{center}
    \begin{tikzpicture}[
      ...
      goldlab/.style={font=\scriptsize, inner sep=1pt, text=accentgold!85!black, fill=white}
    ]
      ...
      % ---- bottom row: calibration cluster ----
      \node[fakgdbox, below=1.5cm of fpdc]  (acs)    {ACS init\\$\hatsig^2(r;\theta)$};
      \node[fakgdbox, below=1.5cm of gain]  (sigma)  {$\hatsig_i^2$ store};
      \node[fakgdbox, below=1.5cm of step]  (mstep)  {EM M-step};

      \draw[arrow] (acs)    -- (sigma);
      \draw[arrow] (sigma)  -- (gain);

      % ---- closed-loop EM feedback (dashed gold) ----
      \draw[feedback] (gain.south east) -- ($(gain.south east)+(0.4,-0.25)$) |- (mstep.north east)
        node[goldlab, pos=0.30, right] {residual $r_i$};
