...
\begin{frame}{Method overview}
  \begin{center}
    \begin{tikzpicture}[
      box/.style={draw=deepnavy, rounded corners=3pt, fill=softgray, very thick,
                  align=center, minimum width=2.25cm, minimum height=0.95cm,
                  font=\small, inner sep=4pt},
      fakgdbox/.style={draw=stanfordred, rounded corners=3pt, fill=white, very thick,
                  align=center, minimum width=2.25cm, minimum height=0.95cm,
                  font=\small, inner sep=4pt},
      arrow/.style={-{Latex[length=2.4mm]}, very thick, draw=deepnavy},
      feedback/.style={-{Latex[length=2.4mm]}, very thick, draw=accentgold, dashed},
      edgelab/.style={font=\scriptsize, inner sep=1pt, fill=white},
      goldlab/.style={font=\tiny, inner sep=1pt, text=accentgold!75!black}
    ]
      % ---- top row: main pipeline ----
      \node[box]                              (y)       {Measured\\k-space $y_\Omega$};
      \node[box,      right=0.85cm of y]      (denoise) {Frozen EDM\\$\muth(x_t,t)$};
      \node[fakgdbox, right=0.85cm of denoise] (fpdc)   {FPDC mask\\$\Omega(t)$};
      \node[box,      right=0.85cm of fpdc]   (gain)    {Kalman gain\\$K_i(t)$};
      \node[box,      right=0.85cm of gain]   (step)    {DDIM step\\$\to x_{t-1}$};
      ...
      % ---- iterate loop above ----
      \coordinate (above) at ($(denoise.north)!0.5!(step.north) + (0,1.05)$);
      \draw[arrow] (step.north) |- (above) -| (denoise.north)
        node[edgelab, pos=0.25, above] {iterate $x_{t-1}\to x_t$};

      % ---- bottom row: pushed left under {y, EDM, FPDC} ----
      \node[fakgdbox, below=1.7cm of y]       (acs)    {ACS init\\$\hatsig^2(r;\theta)$};
      \node[fakgdbox, below=1.7cm of denoise] (sigma)  {$\hatsig_i^2$ store};
      \node[fakgdbox, below=1.7cm of fpdc]    (mstep)  {EM M-step};

      \draw[arrow] (acs) -- (sigma);
      \draw[arrow] (sigma.north east) -- (gain.south west);

      % ---- closed-loop EM feedback (dashed gold, straight diagonals) ----
      \draw[feedback] (gain.south) -- (mstep.north east)
        node[goldlab, midway, above, sloped] {residual $r_i$};
      \draw[feedback] (mstep.west) -- (sigma.east)
        node[goldlab, midway, below] {refresh};
    \end{tikzpicture}
  \end{center}
  \vspace{0.2em}
  \footnotesize
